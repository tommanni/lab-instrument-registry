class Token {
    constructor(value, type, operation, start = 0, end = 0, fieldName) {
      this.value = value
      this.type = type
      if (operation) {
    ...
    static create(value, type, currentPosition, operation, fieldName) {
      const startPosition = calcStart(currentPosition, value.length)
      const endPosition = calcEnd(startPosition, value.length)
      return new Token(value, type, operation, startPosition, endPosition, fieldName)
    }
  }
